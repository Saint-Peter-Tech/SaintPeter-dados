...
def client(df, cursor):
   ...
   # ======= DASHBOARD DIEGO SEITI ========
    ...
    for modulo in modulosLista:
        ...
        somaScores += score * ativos
        somaPesos += ativos

    if somaPesos > 0:

        estabilidadeGeral = round(
            somaScores / somaPesos,
            2
        )

    else:

        estabilidadeGeral = 100


    meta = 80

    if estabilidadeGeral >= meta:
    ...
    monitores_unidade = [int(m) for m in jsonUnidade["listaMonitores"].keys()]

    df_unidade = df.copy()
    df_unidade = df_unidade[df_unidade["id_monitor"].isin(monitores_unidade)]

    df_soma = (
        df_unidade
        .groupby("timestamp", as_index=False)["banda_larga"]
        .min()
...
def lambda_handler(event, context):
    ...
    return {
        "statusCode": 200,
        "body": "ETL executada com sucesso"
    }
